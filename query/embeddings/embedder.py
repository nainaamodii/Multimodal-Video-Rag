...
Example:
    Basic usage::
        ...
        from EduQuery import EduQueryEmbedder, FrameExtractor
        
        # Extract frames first
        frames = FrameExtractor().extract("video.mp4", transcript)
        
        # Generate embeddings
        embedder = EduQueryEmbedder(device="cuda")
...
from ingestion.frame_extractor import ExtractedFrame
from ingestion.transcript_extractor import TranscriptSegment


class EduQueryEmbedder:
    ...
    def __init__(
        ...
        logger.info(f"Initializing EduQueryEmbedder on {self.device}")

        # FIX BUG-3: text_weight and image_weight were missing, causing
        # EduQueryVectorStore.search_by_text to raise AttributeError.
        # Values match design spec §1.5 (slide-heavy content weighting).
        self.text_weight: float = 0.85
        self.image_weight: float = 0.15
    ...
    def embed_text_clip(self, text: str) -> np.ndarray:
        """Generate CLIP text embedding for text.
        
        Uses CLIP's text encoder to create an embedding that can be compared
        with image embeddings. This is different from embed_text() which uses
        ...
        Returns:
            Embedding vector as numpy array. Dimension is 512 for CLIP models.
        ...
        Example:
            >>> embedder = EduQueryEmbedder()
            >>> emb = embedder.embed_text_clip("Click the export button")
            >>> print(emb.shape)
            (512,)
        """
        self._load_vision_model()
        ...
        # Process text
        inputs = self._vision_processor(text=[text], return_tensors="pt", padding=True)
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        
        with torch.no_grad():
            text_features = self._vision_model.get_text_features(**inputs)
